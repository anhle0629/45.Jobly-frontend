...
class JoblyApi {
  ...
  //get comapnies information(filter by name if not undefined)
  static async getCompanies(name){
    let res = await this.request('companies', {name})
    return res.company; 
  }

  //get job information(filter by name if not undefined)
  ...
  static async saveProfile (username, data){
    let response = await this.request(`users/${username}`, data)
    return res.user; 
  }
}
